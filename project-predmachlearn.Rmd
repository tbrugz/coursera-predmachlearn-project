...
Now let's show the trained model
...
We can see that the OOB estimate of error rate is just `1.24%` and an accuracy of `r modFit$results$Accuracy[2]`, which seems quite good.
...
plot <- ggplot(cfdf)
plot + geom_tile(aes(x=Prediction, y=Reference, fill=Freq)) + scale_fill_gradient(low="white", high="blue")
```
